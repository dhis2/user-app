Feature: The user list can be searched
    ...
    Scenario: The user list gets filtered by name
        ...
        Then only the users whos name or display name contains the search term should be displayed

    Scenario: The user list gets filtered by organisation unit
        Given some users exist
        And the implementer navigated to the user list view
        When the implementer searches the list by selecting an organisation unit
        Then only the users whos name or display name contains the search term should be displayed

    Scenario: The user list gets filtered by inactivity
        Given some users exists that have been inactive for some time
        And the implementer navigated to the user list view
        When the implementer chooses an inactivity time
    ...
    Scenario: The user list gets filtered by self registration
        Given some users exists that have been inactive for some time
        And the implementer navigated to the user list view
        When the implementer chooses to view only self-registered users
    ...
    Scenario: The user returns to the search results after opening the edit form
        ...
        When the implementer edits one of the displayed users
