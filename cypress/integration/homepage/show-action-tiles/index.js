...
When(
    ...
    () => {
        cy.findByRole('nav').find('+ div > div:nth-child(3) button:first-child').click()
    }
)

When('the user-manager clicks on the add user icon of the user tile', () => {
    cy.findByRole('nav').find('+ div > div:nth-child(1) button:first-child').click()
})

When(
    'the user-manager clicks on the add user role icon of the user role tile',
    () => {
        cy.findByRole('nav').find('+ div > div:nth-child(2) button:first-child').click()
    }
)
